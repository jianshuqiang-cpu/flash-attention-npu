...
def generate_compressed(
    doc_lens_per_batch: list[list[int]],
    prefix_len: int,
    device: torch.device | None = None,
) -> torch.Tensor:
    """maskr (B, max_Sq) int32: per-query LOCAL right boundary (k >= maskr[q] masked).

    TND: per-batch doc structure, ragged q_len. maskr is in local k coordinates
    [0, q_len_b]. Padding rows (>= q_len_b) are 0 (never read: the kernel only
    iterates valid q tiles per batch). The legacy mask_start/mask_len outputs are
    dropped (they were unused and incomplete for prefix_len>0); the left hole is
    carried by `holes` in build_anymask_params instead."""
    B = len(doc_lens_per_batch)
    seq_lens = [sum(dl) for dl in doc_lens_per_batch]
    max_sq = max(seq_lens)
    maskr = torch.zeros((B, max_sq), dtype=torch.int32, device=device)
    for b, dl in enumerate(doc_lens_per_batch):
        s = seq_lens[b]
        doc_offsets = [0]
        for d in dl:
            doc_offsets.append(doc_offsets[-1] + d)
        row = torch.zeros(s, dtype=torch.int32, device=device)
        for doc_idx, doc_len in enumerate(dl):
            start = doc_offsets[doc_idx]
            end = start + doc_len
            prefix_end = start + min(prefix_len, doc_len)
            q_idx_doc = torch.arange(start, end, device=device)
            row[start:prefix_end] = end
            row[prefix_end:end] = q_idx_doc[prefix_end - start:] + 1
        maskr[b, :s] = row
    return maskr
...
def build_anymask_params(
    ...
    """Build padded [B, max_Sq] / [B, max_Tq, Wk] AnyMask inputs for TND.

    Reproduces generate_dense_mask exactly via:
      * maskr           (B, max_Sq)     int32 : per-query local right boundary.
      * holel/holes     (B, max_Sq, 1)  int32 : one hole per query = [0, prefix_end_of_doc).
      * sparse_compute  (B, max_Tq, Wk) int32 : fully-masked tiles (skip).
      * sparse_mask     (B, max_Tq, Wk) int32 : partial tiles (apply maskr + holes).
      * hole_num        (1,)            int16 : Hn = 1.
    All batched+padded; the device indexes [b, local_q] and skips padding. Hn=1: the
    left hole [0, prefix_end) covers cross-doc + own-prefix; maskr is the right bound."""
    B = len(doc_lens_per_batch)
    seq_lens = [sum(dl) for dl in doc_lens_per_batch]
    holes = torch.zeros((B, max_sq, 1), dtype=torch.int32, device=device)
    for b, dl in enumerate(doc_lens_per_batch):
        s = seq_lens[b]
        doc_offsets = [0]
        for d in dl:
            doc_offsets.append(doc_offsets[-1] + d)
        pe = torch.zeros(s, dtype=torch.int32, device=device)
        for doc_idx, doc_len in enumerate(dl):
            start = doc_offsets[doc_idx]
            prefix_end = start + min(prefix_len, doc_len)
            pe[start:start + doc_len] = prefix_end
        holes[b, :s, 0] = pe
    holel = torch.zeros((B, max_sq, 1), dtype=torch.int32, device=device)
    ...
    return dict(
        ...
        maskr=maskr.to(torch.int32),
        holel=holel,
        holes=holes,
    )


def _dense_mask_one_batch(doc_lens, prefix_len, device):
    """(S, S) bool doc+prefix mask for one batch (True=masked). Local q/k indices."""
...
def generate_dense_mask(
    ...
    """(B, max_Sq, max_kv) bool dense mask, padded True (masked).

    TND: each batch has its own doc structure (ragged q_len); q_len_b == kv_len_b.
    The per-batch (S_b, S_b) mask is placed top-left and the rest padded True so
    padding rows/cols are safely masked (never attended)."""
    B = len(doc_lens_per_batch)
    seq_lens = [sum(dl) for dl in doc_lens_per_batch]
    max_sq = max(seq_lens)
    out = torch.ones((B, max_sq, max_sq), dtype=torch.bool, device=device)
    for b, dl in enumerate(doc_lens_per_batch):
        s = seq_lens[b]
        out[b, :s, :s] = _dense_mask_one_batch(dl, prefix_len, device)
    return out
...
if __name__ == "__main__":
    ...
    # TND: per-batch ragged doc structure (different q_len_b) to exercise the AnyMask
    # tile-stride fix (maxQsBlockNum != per-batch qsBlockNum). q_len_b == kv_len_b.
    doc_lens_per_batch = [
        [128, 96, 160],         # S=384, Tq=3
        [128, 96],              # S=224, Tq=2
        [160, 128, 96, 160],    # S=544, Tq=5  (max)
        [96, 160, 128],         # S=384, Tq=3
    ]
    prefix_len = 32
    num_heads = 8
    kv_heads = 2
    ...
    seq_lens = [sum(dl) for dl in doc_lens_per_batch]
    batch_size = len(doc_lens_per_batch)
    total_q = sum(seq_lens)
    max_sq = max(seq_lens)                         # == max_seqlen_q
    assert max_sq % 16 == 0, ("max_seqlen_q must be 16-aligned for AnyMask "
                              "(mha_fwd.cpp 32B maskr/holel/holes DataCopy)")

    dense_mask = generate_dense_mask(doc_lens_per_batch, prefix_len, device=device)   # (B, max_sq, max_sq)
    maskr = generate_compressed(doc_lens_per_batch, prefix_len, device=device)        # (B, max_sq)
    block_compute, block_mask, tile_range = generate_block_mask(dense_mask, tile_m=tile_m, tile_n=tile_n)
    anymask = build_anymask_params(doc_lens_per_batch, prefix_len, max_sq,
                                   maskr, block_compute, block_mask, device=device)

    # Packed TND Q (total_q,H,D) + packed KV (total_kv,Hkv,D). q_len_b == kv_len_b.
    Q = torch.randn((total_q, num_heads, head_dim), dtype=torch.float32, device=device)
    K = torch.randn((total_q, kv_heads, head_dim), dtype=torch.float32, device=device)
    V = torch.randn((total_q, kv_heads, head_dim), dtype=torch.float32, device=device)

    cu = [0]
    for s in seq_lens:
        cu.append(cu[-1] + s)
    cu_seqlens_q = torch.tensor(cu, dtype=torch.int32, device=device)              # (B+1,) cumulative
    cache_seqlens_cum = cu_seqlens_q.clone()                                       # TND: cumulative kv via seqused_k
    kv_lens_per_batch = torch.tensor(seq_lens, dtype=torch.int32, device=device)   # (B,) per-batch (for golden)
    print(kv_lens_per_batch)
    print(cache_seqlens_cum)
    ...
    if npu_dev is not None:
        torch.set_printoptions(threshold=float("inf"), linewidth=300, precision=5)
        ...
        out = flash_attn_with_kvcache(
            q_npu, k_npu, v_npu,
            cache_seqlens=kv_lens_per_batch.to(npu_dev),
            max_seqlen_q=max_sq,
            softmax_scale=1.0 / math.sqrt(head_dim),
            causal=False,
            window_size=(-1, -1),
            return_softmax_lse=False,
            cu_seqlens_q=cu_seqlens_q.to(npu_dev),
            **am,
        )

        # ---- golden (TND-aware: packed Q/KV + cu_seqlens_q + per-batch kv lens) ----
        golden_out_fp32, _ = golden_flash_attn(
            Q, K, V, kv_lens_per_batch, maskTensor=dense_mask, cu_seqlens_q=cu_seqlens_q,
            causal=False, scale=1.0 / math.sqrt(head_dim),
        )
        golden_out_fp16, _ = golden_flash_attn(
            Q.to(op_dtype), K.to(op_dtype), V.to(op_dtype), kv_lens_per_batch,
            maskTensor=dense_mask, cu_seqlens_q=cu_seqlens_q,
            causal=False, scale=1.0 / math.sqrt(head_dim),
        )

        print(f"batch_size={batch_size} seq_lens={seq_lens} max_sq={max_sq} total_q={total_q}")
        rtol = 1e-3
        atol = rtol
        max_err = (out.cpu() - golden_out_fp16).abs().max().item()
        mean_err = (out.cpu() - golden_out_fp16).abs().mean().item()
        print("\n========================== TND golden_out_fp16 ==========================")
        print(f"out vs golden_out_fp16:  max_err={max_err:.6f}  mean_err={mean_err:.6f} ")
        max_err = (out.cpu() - golden_out_fp32).abs().max().item()
        mean_err = (out.cpu() - golden_out_fp32).abs().mean().item()
        print("\n========================== TND golden_out_fp32 ==========================")
        print(f"out vs golden_out_fp32:  max_err={max_err:.6f}  mean_err={mean_err:.6f} ")
        try:
            torch.testing.assert_close(out.cpu(), golden_out_fp16, rtol=rtol, atol=atol)
            print("PASS (assert_close vs golden_out_fp16)")
        except Exception as e:
            print(f"FAIL (assert_close): {e}")
            if max_err <= 4e-2 and mean_err <= 1e-3:
                print("loose-tolerance PASS (max<=4e-2, mean<=1e-3)")
            else:
                print("CHECK: errors exceed loose tolerance")
