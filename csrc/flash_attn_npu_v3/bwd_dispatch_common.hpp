...
//
// Shared declarations/macros for the backward FAGGeneral dispatch, included by
// the generated autogen/bwd_dispatch_<dtype>_<layout>.cpp stubs so each
// (dtype, layout) TU's 16 kernel instantiations compile in parallel. Keeping the
// launch boilerplate in this header means the stub files cannot drift from each
// other.
...
// the dtype-specific mask x deterministic x headdim selection. This template is
// explicitly instantiated (per dtype + layout) in exactly one autogen TU each,
// so each (dtype, layout) combo's FAGGeneral variants land in their own .o.
