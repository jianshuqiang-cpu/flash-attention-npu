...
def test_fa_custom_ops(data_type, batch_size, num_heads, kv_heads, q_seqlen, kv_seqlen, head_size, cache_mode, block_size, is_causal, layout, is_varied, window_size_left, window_size_right, num_splits):
    ...
    block_tables_cpu = block_tables.cpu() if block_tables is not None else None
    for i in range(batch_size):
        ...
        if layout == "BSND":
            query_cpu_per_batch = query_cpu_all[i]
            if cache_mode == 1:
                block_table = block_tables_cpu[i]
                key_cache_per_batch, value_cache_per_batch = reconstruct_paged_kv(key_cache_cpu, value_cache_cpu, block_table, kv_seqlen_per_batch, block_size)
            else:
                key_cache_per_batch = key_cache_cpu[i]
                value_cache_per_batch = value_cache_cpu[i]
        else:
            query_cpu_per_batch = query_cpu_all[new_q_seqlen_list_cpu[i] : new_q_seqlen_list_cpu[i + 1]]
            if cache_mode == 0:
        ...
        apply_swa_correction(out_gpu, golden_lse_gpu, q_seqlen_per_batch, kv_seqlen_per_batch, window_size_left_golden, window_size_right_golden, is_local_golden)
        apply_swa_correction(out_plain, lse_plain, q_seqlen_per_batch, kv_seqlen_per_batch, window_size_left_golden, window_size_right_golden, is_local_golden)
        if layout == "BSND":
