...
// AnyMask wiring fix: forward-declare the primary BlockEpilogue template (this
// file is a partial specialization). See online_softmax.hpp for rationale.
...
public:
                                                ...
                                                uint32_t colTail, uint32_t vlElemNum)
    {
        ...
        RegTensor<float> goCurRollVreg;
        RegTensor<float> glVreg;
        RegTensor<float> glSafeVreg;
        RegTensor<float> tinyVreg;
        RegTensor<float> divVreg;
        RegTensor<float> divRollVreg;
        MaskReg pregFull = CreateMask<float, MaskPattern::ALL>();
        MaskReg pregTail = UpdateMask<float>(colTail);
        // AnyMask (v1, D5): guard div-by-zero for fully-masked rows. expSum (glVreg)
        // is 0 when a Q row has no valid KV; O_accum is also 0 there, so 0/tiny=0.
        // For normal rows expSum >> tiny, so Max is a no-op. NOTE: fully-masked-row
        // correctness also depends on the epilogue's rowmax==MIN_VALUE path; verify
        // on 950 (RescaleFuncLastNotFirst may need the same guard).
        Duplicate(tinyVreg, 1e-38f);
        for (uint32_t i = 0; i < row; i++) {
            LoadAlign<T, LoadDist::DIST_BRC_B32>(glVreg, glUb + i);
            Max(glSafeVreg, glVreg, tinyVreg, pregFull);
            ...
            Div(divVreg, goCurVreg, glSafeVreg, pregFull);
            Div(divRollVreg, goCurRollVreg, glSafeVreg, pregFull);
            StoreAlign<T, StoreDist::DIST_NORM_B32>(goUb + i * VHeadSize, divVreg, pregFull);
